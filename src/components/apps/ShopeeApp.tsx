...
export const ShopeeApp: React.FC = () => {
    ...
    return (
        <div className="flex flex-col h-full bg-slate-100 font-sans overflow-auto">
            {/* Header */}
            <div className="bg-gradient-to-b from-[#f53d2d] to-[#f63] text-white p-2 sm:p-3 sticky top-0 z-10">
                <div className="flex items-center gap-2 sm:gap-3 mb-2 sm:mb-3">
                    <div className="flex-1 bg-white rounded-sm flex items-center p-1 sm:p-1.5 gap-2">
                        <Search size={14} className="sm:w-4 sm:h-4 text-slate-400" />
                        <input
                            type="text"
                            placeholder="Sign up and get 100% off"
                            className="flex-1 text-xs sm:text-sm text-black outline-none placeholder:text-[#f53d2d]"
                            value={searchQuery}
            ...
            <div className="flex-1 overflow-auto">
                ...
                {!searchQuery && (
                    <>
                        ...
                        {/* Categories / Icons */}
                        <div className="bg-white p-3 sm:p-4 grid grid-cols-5 gap-2 sm:gap-4 mb-2">
                            {[
                        ...
                        {/* Flash Sale */}
                        <div className="bg-white p-2 sm:p-3 mb-2">
                            <div className="flex items-center justify-between mb-2 sm:mb-3">
                                <div className="flex items-center gap-2">
                                    <span className="text-orange-500 font-bold italic text-base sm:text-lg">FLASH SALE</span>
                                    <div className="flex gap-1 text-white text-[10px] sm:text-xs font-bold">
                                ...
                                <span className="text-slate-500 text-[10px] sm:text-xs">See All &gt;</span>
                            </div>
                            <div className="flex gap-2 sm:gap-3 overflow-x-auto pb-2">
                                {shop.products.slice(0, 5).map((product) => (
                                    <div key={`flash-${product.id}`} className="w-20 sm:w-24 shrink-0 flex flex-col gap-1">
                                        <div className="w-20 h-20 sm:w-24 sm:h-24 bg-slate-100 rounded relative">
                                            <div className="absolute top-0 right-0 bg-yellow-400 text-red-600 text-[10px] font-bold px-1">
                                                -50%
                                            </div>
                                        </div>
                                        <div className="text-orange-500 text-xs sm:text-sm font-bold">
                                            ₱{formatMoney(product.cost).replace('$', '')}
                                        </div>
                                        <div className="w-full h-2 sm:h-3 bg-red-100 rounded-full relative overflow-hidden">
                                            <div className="absolute left-0 top-0 h-full bg-red-500 w-3/4" />
                                            <span className="absolute inset-0 text-[7px] sm:text-[8px] text-white flex items-center justify-center font-bold">
                                                SOLD 12
                                            </span>
                                        </div>
                                    </div>
                ...
                <div className="p-1 sm:p-2">
                    <div className="text-orange-500 font-bold mb-2 sticky top-0 bg-slate-100 py-2 z-10 text-xs sm:text-base">
                            ...
                            return (
                                <div key={product.id} className="bg-white rounded shadow-sm overflow-hidden flex flex-col">
                                    <div className="aspect-square bg-slate-200 relative">
                                        {/* Image placeholder */}
                                        <div className="absolute bottom-0 left-0 bg-[#f53d2d] text-white text-[9px] sm:text-[10px] px-1">
                                            Mall
                                        </div>
                                    </div>
                                    <div className="p-1.5 sm:p-2 flex-1 flex flex-col">
                                        <div className="text-[10px] sm:text-xs line-clamp-2 mb-1 sm:mb-2 text-slate-800">
                                            {product.name} - {product.description}
                                        </div>
                                        <div className="mt-auto">
                                            <div className="flex items-baseline gap-1">
                                                <span className="text-[9px] sm:text-[10px] text-orange-500">₱</span>
                                            ...
                                            <div className="flex items-center justify-between mt-1">
                                                ...
                                                <span className="text-[9px] sm:text-[10px] text-slate-500">10k sold</span>
                                            </div>
                                        </div>
                                        <button
                                            ...
                                            disabled={!canAfford}
                                            className={clsx(
                                                "mt-1.5 sm:mt-2 w-full py-1 text-[10px] sm:text-xs font-bold text-white rounded shadow-sm active:scale-95 transition-transform",
