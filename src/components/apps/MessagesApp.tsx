...
export const MessagesApp: React.FC = () => {
    return (
        <div className="flex h-full bg-white font-sans">
            {/* Sidebar */}
            <div className="w-80 border-r border-slate-200 flex flex-col bg-slate-50/50 backdrop-blur-xl">
                <div className="p-3 border-b border-slate-200 flex items-center justify-between">
                    ...
                    <button className="ml-2 text-blue-500"><Edit size={18} /></button>
                </div>
                <div className="flex-1 overflow-y-auto">
                    {MESSAGES.map((msg) => (
                        <div key={msg.id} className={`p-3 border-b border-slate-100 hover:bg-blue-50 cursor-pointer ${msg.unread ? 'bg-blue-50/30' : ''}`}>
                            ...
                            {msg.unread && <div className="w-2 h-2 bg-blue-500 rounded-full mt-2" />}
                        </div>
                    ))}
                </div>
            </div>
            ...
            <div className="flex-1 flex flex-col bg-white">
                <div className="h-14 border-b border-slate-100 flex items-center justify-between px-4">
                    <div className="flex flex-col">
                        <span className="font-bold text-sm">BankSim</span>
                        <span className="text-xs text-slate-400">To: You</span>
                    </div>
                ...
                <div className="flex-1 p-4 overflow-y-auto flex flex-col gap-4">
                    <div className="self-center text-xs text-slate-400 my-2">Today 9:41 AM</div>
                    <div className="self-start bg-slate-200 rounded-2xl rounded-tl-none px-4 py-2 max-w-xs text-sm text-slate-800">
                        Your account balance is low. Please top up immediately to avoid service interruption.
                    </div>
                    <div className="self-end bg-blue-500 text-white rounded-2xl rounded-tr-none px-4 py-2 max-w-xs text-sm">
                        I just bought a private island, what do you expect?
                    </div>
                    <div className="self-start bg-slate-200 rounded-2xl rounded-tl-none px-4 py-2 max-w-xs text-sm text-slate-800">
                        Your payment of $50,000 to Luxury District was successful.
                    </div>
                    <div className="self-end text-xs text-slate-400 mr-2">Delivered</div>
                </div>
                <div className="p-4 border-t border-slate-100">
                    <div className="border border-slate-300 rounded-full px-4 py-2 flex items-center gap-2">
                        <input type="text" placeholder="iMessage" className="flex-1 outline-none text-sm" />
