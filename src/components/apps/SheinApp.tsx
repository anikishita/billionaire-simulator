...
export const SheinApp: React.FC = () => {
    ...
    const shop = state.shops.find((s) => s.id === 'luxury'); // Using Luxury for Shein
                        ...
                        return (
                            <div key={`grid-${product.id}`} className="relative group">
                                <div className="aspect-[3/4] bg-slate-100 relative">
                                    <button className="absolute bottom-2 right-2 bg-white/80 p-1.5 rounded-full opacity-0 group-hover:opacity-100 transition-opacity" onClick={() => buyProduct(shop.id, product.id)} disabled={!canAfford}>
                                        <ShoppingBag size={16} />
                                    </button>
                                </div>
                                <div className="p-2">
